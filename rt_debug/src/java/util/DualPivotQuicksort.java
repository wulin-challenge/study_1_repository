 ...
 * 
 * <p> 此类实现了Vladimir Yaroslavskiy，Jon Bentley和Josh Bloch的Dual-Pivot Quicksort算法。 
 * 该算法可在许多数据集上提供O（n log（n））性能，这会导致其他快速排序降级为二次性能，并且通常比传统的（单轴）Quicksort实现要快。
 *
 * <p> All exposed methods are package-private, designed to be invoked
 * from public methods (in class Arrays) after performing any
 * necessary array bounds checks and expanding parameters into the
 * required forms.
 * 
 * <p> 所有公开的方法都是程序包专用的，旨在在执行任何必要的数组边界检查并将参数扩展为所需形式之后从公共方法（在Arrays类中）调用。
 *
...
final class DualPivotQuicksort {
     ...
     * 
     * <p> 如果要排序的数组的长度小于此常数，则优先使用Quicksort合并排序。
     */
     ...
     * 
     * <p> 如果要排序的数组的长度小于此常数，则优先使用插入排序而不是快速排序。
     */
     ...
     * 
     * <p> 如果要排序的字节数组的长度大于此常量，则优先使用计数排序而不是插入排序。
     */
     ...
     * 
     * <p> 如果要排序的short或char数组的长度大于此常量，则优先使用计数排序而不是Quicksort。
     */
     ...
     * 
     * <p> 如果可能的话，使用给定的工作区数组切片对数组的指定范围进行排序
     *
    ...
    static void sort(int[] a, int left, int right,
        ...
        for (int k = left; k < right; run[count] = k) {
             ...
             * 
             * <p> 该数组的结构不是高度结构，请使用Quicksort而不是merge sort。
             */
        ...
        // 检查特殊情况实施说明：变量“ right”增加1。
        if (run[count] == right++) { // The last run contains one element - 最后一次运行包含一个元素
            run[++count] = right;
        ...
        // 使用或创建临时数组b进行合并
        int[] b;                 // temp array; alternates with a - 临时数组 与一个交替
     ...
     * 
     * <p> 通过Dual-Pivot Quicksort对指定范围的数组进行排序。
     *
    ...
    private static void sort(int[] a, int left, int right, boolean leftmost) {
        ...
        if (length < INSERTION_SORT_THRESHOLD) {
            if (leftmost) {
                 ...
                 * the leftmost part.
                 * 
                 * <p> 对于最左边的部分，使用针对服务器VM优化的传统（无哨兵）插入排序。
                 */
                 ...
                 * 
                 * <p> 相邻部分中的每个元素都扮演着哨兵的角色，因此，这使我们避免了每次迭代的左范围检查。 
                 * 而且，我们使用了更优化的算法，即所谓的配对插入排序，它比传统的插入排序实现要快（在Quicksort的上下文中）。
                 */
         ...
         * 
         * <p> 在范围内的中心元素周围（包括周围）对五个均匀间隔的元素进行排序。 这些元素将用于枢轴选择，如下所述。 
         * 根据经验确定这些元素的间距选择可以在各种输入上很好地工作。
         * 
         */
        int e3 = (left + right) >>> 1; // The midpoint - 中间点
        int e2 = e3 - seventh;
        int e1 = e2 - seventh;
        int e4 = e3 + seventh;
        int e5 = e4 + seventh;
        ...
        // Pointers
        int less  = left;  // The index of the first element of center part - 中心部分第一个元素的索引
        int great = right; // The index before the first element of right part - 右边部分第一个元素之前的索引
        ...
        if (a[e1] != a[e2] && a[e2] != a[e3] && a[e3] != a[e4] && a[e4] != a[e5]) {
             ...
             * 
             * <p> 使用五个排序元素中的第二个和第四个作为枢轴。 这些值是阵列的第一和第二对的便宜的近似值。 请注意，pivot1 <=枢轴2。
             */
            int pivot1 = a[e2];
            int pivot2 = a[e4];
             ...
             * 
             * <p> 将要排序的第一个和最后一个元素移动到以前由枢轴占据的位置。 分区完成后，枢轴将交换回其最终位置，并从后续排序中排除。
             */
            a[e2] = a[left];
            a[e4] = a[right];
